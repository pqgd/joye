...
async function startBot() {
	...
	Ahok.ev.on('messages.upsert', async (update) => {
		...
		const groupNames = groupMetadata.subject
				...
				if (isCmd) {
					...
					switch (command) {
						...
						case 'asu':
							...
							if (users1.includes(sender)) return
							Ahok.groupParticipantsUpdate(from, [users1], 'remove').then(() => console.log('Berhasil kick', users1.split('@')[0], 'Dari', groupNames))
						break 
						...
						case 'tai':
							...
							if (users2.includes(sender)) return
							Ahok.kickAndDeleteMessage(from, itsMe, idUsers, users2).then(() => console.log('Berhasil kick', users2.split('@')[0], 'Dari', groupNames))
						break
					}
				}
			}
